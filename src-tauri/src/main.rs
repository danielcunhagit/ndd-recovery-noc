...
async fn handle_login(provider: String, email: String, password: String) -> Result<String, String> {
    ...
    let body_enterprises = format!(
        ...
        </soap:Envelope>"#, provider, email, encrypted_password
    );

    let res = client.post("https://api-general.nddprint.com/GeneralWS/GeneralData2.asmx")
        .header("Content-Type", "text/xml; charset=utf-8")
        .body(body_enterprises).send().await.map_err(|_| "Falha ao conectar aos servidores da NDD. Verifique sua internet.".to_string())?;

    let xml_response = res.text().await.map_err(|_| "Resposta inválida do servidor.".to_string())?;
...
fn process_excel_file(app: AppHandle, file_path: String) -> Result<String, String> {
    ...
    let mut workbook: Xlsx<_> = open_workbook(&file_path).map_err(|e| format!("Erro ao abrir Excel: {}", e))?;
    let sheet_names = workbook.sheet_names().to_owned();
    
    if sheet_names.is_empty() { return Err("A planilha não possui abas.".to_string()); }
    let first_sheet = &sheet_names[0];

    if let Ok(range) = workbook.worksheet_range(first_sheet) {
        let tx = conn.transaction().map_err(|e| e.to_string())?;
        tx.execute("DELETE FROM contatos", []).map_err(|e| e.to_string())?;
        ...
        for row in rows {
            let get_val = |idx: usize| -> String { row.get(idx).map(|c| c.to_string()).unwrap_or_default() };
            stmt.execute(params![
                get_val(0), get_val(1), get_val(2), get_val(3), get_val(4), 
                get_val(5), get_val(6), get_val(7), get_val(8), get_val(9)
            ]).map_err(|e| e.to_string())?;
        }
        drop(stmt);
...
#[tauri::command]
async fn fetch_ndd_data(app: AppHandle, provider: String, email: String, password: String) -> Result<NddResult, String> {
    let encrypted_password = AuthManager::encrypt_password(&password);
    let client = Client::builder().danger_accept_invalid_certs(true).build().map_err(|e| e.to_string())?;

    app.emit("ndd-progress", ProgressPayload { message: "Conectando à NDD e buscando empresas...".to_string(), progress: 5.0, printers: 0, companies: 0 }).unwrap();
    ...
    let res = client.post("https://api-general.nddprint.com/GeneralWS/GeneralData2.asmx")
        .header("Content-Type", "text/xml; charset=utf-8").body(body_enterprises).send().await.map_err(|e| e.to_string())?;

    let xml_response = res.text().await.map_err(|e| e.to_string())?;
    
    let open_tag = "<GetEnterprisesResult>";
    let close_tag = "</GetEnterprisesResult>";
    let json_str = if let (Some(start), Some(end)) = (xml_response.find(open_tag), xml_response.find(close_tag)) {
        &xml_response[start + open_tag.len()..end]
    } else { return Err("Falha ao extrair GetEnterprisesResult do XML".to_string()); };

    let parsed_json: serde_json::Value = serde_json::from_str(json_str).map_err(|e| e.to_string())?;
    let mut enterprises = parsed_json.as_array().ok_or("Formato de empresas inválido")?.clone();
    ...
    for enterprise in enterprises {
        ...
        let ent_name = enterprise["EnterpriseName"].as_str().unwrap_or("Desconhecida").to_string();
        ...
        let task = tokio::spawn(async move {
            ...
            let body_printers = format!(
                ...
                </soap:Envelope>"#, provider, email, encrypted_password, ent_id
            );

            let res_printers = client.post("https://api-general.nddprint.com/GeneralWS/GeneralData2.asmx")
                .header("Content-Type", "text/xml; charset=utf-8").body(body_printers).send().await;

            if let Ok(res) = res_printers {
                if let Ok(xml_printers) = res.text().await {
                    if let (Some(start), Some(end)) = (xml_printers.find("<GetPrintersResult>"), xml_printers.find("</GetPrintersResult>")) {
                        let printers_json_str = &xml_printers[start + 19..end];
                        if let Ok(mut printers_arr) = serde_json::from_str::<Vec<serde_json::Value>>(printers_json_str) {
            ...
            let total_printers_now = { let lock = all_printers.lock().await; lock.len() };

            let _ = app.emit("ndd-progress", ProgressPayload { 
                message: format!("{} ({}/{})", ent_name, count, total_enterprises), 
                progress: current_progress, printers: total_printers_now, companies: count 
            });
        });
        tasks.push(task);
    }
    ...
    let total_impressoras = { let lock = all_printers.lock().await; lock.len() };
    app.emit("ndd-progress", ProgressPayload { 
        message: "Cruzando dados com o Banco Local...".to_string(), progress: 95.0, printers: total_impressoras, companies: total_enterprises
    }).unwrap();
    ...
    
    {
        ...
        for row in rows { if let Ok((name, json_data)) = row { contacts_map.insert(name.to_uppercase().trim().to_string(), json_data); } }
    } 

    let mut processed_printers = Vec::new();
    let data_atual = Utc::now().naive_utc();
    let mut count_2001 = 0; let mut count_esporadico = 0; let mut count_match_sqlite = 0;
    ...
    let mut drop_disabled_printers = 0; let mut drop_brand_printers = 0;
    ...
    for p in final_printers.iter() {
        ...
        if !printer["EnabledCounters"].as_bool().unwrap_or(false) { drop_disabled_printers += 1; continue; }
        unique_after_disabled.insert(ent_name_upper.clone()); 

        let brand = printer["BrandName"].as_str().unwrap_or("");
        if !["Canon", "Brother", "Oce", "OKI"].contains(&brand) { drop_brand_printers += 1; continue; }
        unique_after_brand.insert(ent_name_upper.clone()); 

        let mut days_offline: i64 = 0;
        let mut is_2001 = false;
        
        if let Some(d) = printer["Days without meters"].as_i64().or_else(|| printer["DaysWithoutMeters"].as_i64()) {
            days_offline = d;
        } else if let Some(date_str) = printer["LastNSLDate"].as_str() {
            if date_str.contains("2001") { is_2001 = true; count_2001 += 1; }
            ...
            let parsed_date = NaiveDateTime::parse_from_str(date_without_ms, "%Y-%m-%d %H:%M:%S")
                ...
                .or_else(|_| NaiveDateTime::parse_from_str(&format!("{} 00:00:00", date_without_ms), "%Y-%m-%d %H:%M:%S"));

            if let Ok(last_date) = parsed_date { days_offline = (data_atual - last_date).num_days(); }
        }

        let mut passivel = "SIM"; let mut motivo = "";
        if is_2001 { passivel = "NÃO"; motivo = "Equipamento não compatível (Data 2001)";
        } else if ["GESTAMP", "DEFENSORIA", "HOSPITAL PAULISTA"].iter().any(|&s| ent_name_upper.contains(s)) { passivel = "NÃO"; motivo = "Cliente Esporádico"; count_esporadico += 1; }

        let (tel, email1, email2, email3, nome1, nome2, nome3, consultor, codigo_empresa) = match contacts_map.get(&ent_name_upper) {
            Some(contact) => {
                count_match_sqlite += 1; 
                (contact["tel"].as_str().unwrap_or(""), contact["email1"].as_str().unwrap_or(""), contact["email2"].as_str().unwrap_or(""), contact["email3"].as_str().unwrap_or(""), contact["nome1"].as_str().unwrap_or(""), contact["nome2"].as_str().unwrap_or(""), contact["nome3"].as_str().unwrap_or(""), contact["consultor"].as_str().unwrap_or(""), contact["codigo_empresa"].as_str().unwrap_or(""))
            },
            None => ("", "", "", "", "", "", "", "", "")
        };

        if let Some(obj) = printer.as_object_mut() {
            obj.insert("Days without meters".to_string(), serde_json::json!(days_offline));
            obj.insert("Passivel de Monitoramento".to_string(), serde_json::json!(passivel));
            obj.insert("Motivo Nao Monitoramento".to_string(), serde_json::json!(motivo));
            obj.insert("tel".to_string(), serde_json::json!(tel)); obj.insert("email1".to_string(), serde_json::json!(email1));
            obj.insert("email2".to_string(), serde_json::json!(email2)); obj.insert("email3".to_string(), serde_json::json!(email3));
            obj.insert("nome1".to_string(), serde_json::json!(nome1)); obj.insert("nome2".to_string(), serde_json::json!(nome2));
            obj.insert("nome3".to_string(), serde_json::json!(nome3)); obj.insert("consultor".to_string(), serde_json::json!(consultor));
            obj.insert("codigo_empresa".to_string(), serde_json::json!(codigo_empresa));
        }
    ...
    app.emit("ndd-progress", ProgressPayload { 
        message: "Processamento concluído!".to_string(), progress: 100.0, printers: final_printers.len(), companies: total_enterprises 
    }).unwrap();
    ...
    Ok(NddResult {
        ...
        stats: FilteringStats {
            initial_printers: final_printers.len(), initial_companies, drop_disabled_printers, drop_disabled_companies, drop_brand_printers, drop_brand_companies, flag_2001: count_2001, flag_sporadic: count_esporadico, matched_sqlite: count_match_sqlite, final_printers: processed_printers.len(), final_companies: unique_after_brand.len(),
        }
    })
}
...
fn get_all_contacts(app: AppHandle) -> Result<Vec<Contact>, String> {
    ...
    for contact in contacts { if let Ok(c) = contact { result.push(c); } }
    Ok(result)
}
...
#[tauri::command]
fn export_contacts_to_excel(app: AppHandle) -> Result<String, String> {
    let contacts = get_all_contacts(app.clone())?;
    let docs_dir = app.path().document_dir().map_err(|e| e.to_string())?;
    let path = docs_dir.join("Base_de_Contatos_NDD.xlsx");
    ...
    let headers = ["EnterpriseName", "tel", "email1", "email2", "email3", "nome1", "nome2", "nome3", "consultor", "codigo_empresa"];
    for (i, title) in headers.iter().enumerate() { worksheet.write_string(0, i as u16, *title, &fmt).map_err(|e| e.to_string())?; }

    for (r_idx, c) in contacts.iter().enumerate() {
        let r = (r_idx + 1) as u32;
        worksheet.write_string(r, 0, &c.enterprise_name, &fmt).unwrap(); worksheet.write_string(r, 1, &c.tel, &fmt).unwrap();
        worksheet.write_string(r, 2, &c.email1, &fmt).unwrap(); worksheet.write_string(r, 3, &c.email2, &fmt).unwrap();
        worksheet.write_string(r, 4, &c.email3, &fmt).unwrap(); worksheet.write_string(r, 5, &c.nome1, &fmt).unwrap();
        worksheet.write_string(r, 6, &c.nome2, &fmt).unwrap(); worksheet.write_string(r, 7, &c.nome3, &fmt).unwrap();
        worksheet.write_string(r, 8, &c.consultor, &fmt).unwrap(); worksheet.write_string(r, 9, &c.codigo_empresa, &fmt).unwrap();
    }
    workbook.close().map_err(|e| e.to_string())?;
    Ok(format!("Base de contatos salva em: {}", path_str))
}
...
async fn export_to_excel(app: AppHandle, data: Vec<serde_json::Value>) -> Result<String, String> {
    ...
    let headers = [
        "PrinterDeviceID", "EnterpriseID", "EnterpriseName", "SiteID", "SiteName", "SiteDivisionName", "PrinterName", "BrandName", "ModelName", "SerialNumber", "AddressName", "Location", "ContactData", "LastNSLDate", "Passivel de Monitoramento", "Motivo Nao Monitoramento", "Days without meters", "Telefone", "Email 1", "Email 2", "Email 3", "Nome 1", "Nome 2", "Nome 3", "Consultor", "Código Empresa"
    ];

    for (i, title) in headers.iter().enumerate() { worksheet.write_string(0, i as u16, *title, &fmt).unwrap(); }

    for (r_idx, item) in data.iter().enumerate() {
        let r = (r_idx + 1) as u32;
        let get_val = |key: &str| -> String { match &item[key] { serde_json::Value::String(s) => s.clone(), serde_json::Value::Number(n) => n.to_string(), _ => "".to_string(), } };
        
        worksheet.write_string(r, 0, &get_val("PrinterDeviceID"), &fmt).unwrap(); worksheet.write_string(r, 1, &get_val("EnterpriseID"), &fmt).unwrap();
        worksheet.write_string(r, 2, &get_val("EnterpriseName"), &fmt).unwrap(); worksheet.write_string(r, 3, &get_val("SiteID"), &fmt).unwrap();
        worksheet.write_string(r, 4, &get_val("SiteName"), &fmt).unwrap(); worksheet.write_string(r, 5, &get_val("SiteDivisionName"), &fmt).unwrap();
        worksheet.write_string(r, 6, &get_val("PrinterName"), &fmt).unwrap(); worksheet.write_string(r, 7, &get_val("BrandName"), &fmt).unwrap();
        worksheet.write_string(r, 8, &get_val("ModelName"), &fmt).unwrap(); worksheet.write_string(r, 9, &get_val("SerialNumber"), &fmt).unwrap();
        worksheet.write_string(r, 10, &get_val("AddressName"), &fmt).unwrap(); worksheet.write_string(r, 11, &get_val("Location"), &fmt).unwrap();
        worksheet.write_string(r, 12, &get_val("ContactData"), &fmt).unwrap(); worksheet.write_string(r, 13, &get_val("LastNSLDate"), &fmt).unwrap();
        worksheet.write_string(r, 14, &get_val("Passivel de Monitoramento"), &fmt).unwrap(); worksheet.write_string(r, 15, &get_val("Motivo Nao Monitoramento"), &fmt).unwrap();
        
        let days = match &item["Days without meters"] { serde_json::Value::Number(n) => n.as_f64().unwrap_or(0.0), serde_json::Value::String(s) => s.parse::<f64>().unwrap_or(0.0), _ => 0.0, };
        worksheet.write_number(r, 16, days, &fmt).unwrap();

        worksheet.write_string(r, 17, &get_val("tel"), &fmt).unwrap(); worksheet.write_string(r, 18, &get_val("email1"), &fmt).unwrap();
        worksheet.write_string(r, 19, &get_val("email2"), &fmt).unwrap(); worksheet.write_string(r, 20, &get_val("email3"), &fmt).unwrap();
        worksheet.write_string(r, 21, &get_val("nome1"), &fmt).unwrap(); worksheet.write_string(r, 22, &get_val("nome2"), &fmt).unwrap();
        worksheet.write_string(r, 23, &get_val("nome3"), &fmt).unwrap(); worksheet.write_string(r, 24, &get_val("consultor"), &fmt).unwrap();
        worksheet.write_string(r, 25, &get_val("codigo_empresa"), &fmt).unwrap();
    }
    workbook.close().map_err(|e| e.to_string())?;
...
fn ensure_contacts_exist(app: AppHandle, names: Vec<String>) -> Result<(), String> {
    let conn = DatabaseManager::get_connection(&app)?;
    for name in names {
        let mut stmt = conn.prepare("SELECT COUNT(*) FROM contatos WHERE UPPER(enterprise_name) = UPPER(?1)").map_err(|e| e.to_string())?;
...
#[tauri::command]
fn send_test_email(user_email: String, user_pass: String, target_email: String) -> Result<String, String> {
...
async fn fetch_gemini_template(client: &reqwest::Client, key: &str, prompt: &str) -> Result<String, String> {
...
fn get_user_profile(app: AppHandle, email: String) -> Result<UserProfile, String> {
    ...
    let mut stmt = conn.prepare("SELECT name, title, department, phone FROM user_profiles WHERE email = ?1").map_err(|e| e.to_string())?;
    let profile = stmt.query_row([&email], |row| {
        Ok(UserProfile { email: email.clone(), name: row.get(0).unwrap_or_default(), title: row.get(1).unwrap_or_default(), department: row.get(2).unwrap_or_default(), phone: row.get(3).unwrap_or_default() })
    }).unwrap_or(UserProfile { email, name: "".to_string(), title: "".to_string(), department: "".to_string(), phone: "".to_string() });
...
async fn process_and_send_emails(
    app: AppHandle, user_email: String, user_pass: String, gemini_key: String, 
    offline_days_threshold: i64, send_to_host_offline: bool, send_to_printer_offline: bool, is_dev_mode: bool, data: Vec<serde_json::Value>
    ...
    let template_host = if send_to_host_offline { fetch_gemini_template(&client, &gemini_key, prompt_host).await? } else { "".to_string() };
    let template_isolated = if send_to_printer_offline { fetch_gemini_template(&client, &gemini_key, prompt_isolated).await? } else { "".to_string() };

    let mut companies_map: std::collections::HashMap<String, Vec<serde_json::Value>> = std::collections::HashMap::new();
    for p in data {
        if p["Passivel de Monitoramento"] == "NÃO" { continue; } 
        let ent_name = p["EnterpriseName"].as_str().unwrap_or("Desconhecida").to_string();
        companies_map.entry(ent_name).or_insert_with(Vec::new).push(p);
    }

    let creds = Credentials::new(user_email.clone(), user_pass);
    let mailer = SmtpTransport::relay("smtp.gmail.com").map_err(|e| e.to_string())?.credentials(creds).build();

    // Busca o perfil do usuário no banco local
    let profile = get_user_profile(app.clone(), user_email.clone()).unwrap_or(UserProfile {
        email: user_email.clone(), name: "Equipe Canon".to_string(), title: "".to_string(), department: "".to_string(), phone: "".to_string()
    });
    ...
    let signature_html = format!(
        ...
        profile.name, profile.title, profile.department, profile.email, profile.email, profile.phone
    );

    let mut emails_enviados = 0; let mut empresas_processadas = 0; 

    for (empresa, impressoras) in companies_map {
        if is_dev_mode && empresas_processadas >= 2 { break; }

        let email_cliente = impressoras[0]["email1"].as_str().unwrap_or("").to_string();
        let nome_cliente = impressoras[0]["nome1"].as_str().unwrap_or("Equipe TI").to_string();
        if email_cliente.is_empty() && !is_dev_mode { continue; }
        ...
        if offline_printers.is_empty() { continue; }
        ...
        if is_host_offline && !send_to_host_offline { continue; }
        if !is_host_offline && !send_to_printer_offline { continue; }

        let _ = app.emit("email-dispatch-event", format!("⚙️ Gerando e-mail para {}...", empresa));

        let base_template = if is_host_offline { &template_host } else { &template_isolated };
        let subject = if is_host_offline { format!("Verificação de Servidor NDD Print Host - {}", empresa) } else { format!("Impressoras sem comunicação com o NDD - {}", empresa) };
        ...
        let target_email = if is_dev_mode { user_email.clone() } else { email_cliente.clone() };
        let subject_final = if is_dev_mode { format!("[TESTE DEV] {}", subject) } else { subject };
        ...
        let image_part = lettre::message::SinglePart::builder()
            ...
            .header(lettre::message::header::ContentId::from(format!("<{}>", "canon_logo.png")))
        ...
        let email_message = match Message::builder()
            ...
            .multipart(multi) { // <-- Manda o pacote com a imagem embutida
                Ok(m) => m, Err(e) => { let _ = app.emit("email-dispatch-event", format!("❌ Erro na montagem: {}", e)); continue; }
            };

        match mailer.send(&email_message) {
            Ok(_) => { emails_enviados += 1; let _ = app.emit("email-dispatch-event", format!("✅ Sucesso: {}", target_email)); },
            Err(e) => { let _ = app.emit("email-dispatch-event", format!("❌ Erro ({target_email}): {e}")); },
        }
        empresas_processadas += 1;
        std::thread::sleep(std::time::Duration::from_millis(1500));
    }

    let _ = app.emit("email-dispatch-event", "🏳️ OPERAÇÃO FINALIZADA.".to_string());
    Ok(format!("{} e-mails processados e disparados!", emails_enviados))
}
...
fn main() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            handle_login, process_excel_file, check_has_contacts, fetch_ndd_data, export_to_excel,
            get_all_contacts, save_contact, export_contacts_to_excel, ensure_contacts_exist, delete_contact, 
            send_test_email, process_and_send_emails, get_user_profile, save_user_profile
        ])
